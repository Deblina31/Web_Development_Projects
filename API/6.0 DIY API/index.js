...
app.post("/jokes", (req, res)=>{
  const newJoke={id:jokes.length+1, jokeText:req.body.jokeText,
...
//8. DELETE All jokes (write above to run properly)
app.delete("/all", (req, res)=>{
  const userKey=req.query.key;
  if(userKey === masterKey){
    jokes=[];
    res.status(200).json({ message: "Dili toh delete kore !!!!" });
  }
  else{
    res.status(404).json({ message: "Bro what are u doing?" });
  }
})
app.listen(port, () => {
